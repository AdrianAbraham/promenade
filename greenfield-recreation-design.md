...
Track JSON files are direct children of the tracks folder with extension `.json` that validate against the track metadata schema.
...
The playlists folder contains app-managed playlist JSON files. Playlist files use extension `.json`; their filenames are generated by Promenade 2.
...
- Calls mute state.
- Auto-mute settings.
- Count-in/playback delay settings.
...
interface PracticePlaybackEngine {
    ...
    suspend fun setPlaybackDelay(settings: PlaybackDelaySettings)
}
```
...
- Count-in/playback delay is handled by the playback service before audio starts, so app controls, Bluetooth controls, and lock-screen play all behave consistently.

### Count-In And Playback Delay

Promenade 2 supports a simple playback delay for practice setup. This is intentionally not a metronome in the initial design.

Rules:

- Delay is configured as off or a fixed duration in seconds.
- Delay applies when starting playback from a ready or paused track at the current position.
- Delay does not apply to seek operations.
- Delay does not apply when resuming after transient audio focus loss.
- During delay, the playback engine emits countdown state while keeping the Media3 session command path active.
- If the user presses pause during delay, playback is cancelled and the track remains at the original position.
- If the user presses next/previous during delay, the delay is cancelled and the new track is loaded paused.
- The UI may show a countdown, but the countdown is derived from playback-engine state rather than implemented as UI-only timing.
...
Initial instructions display may start as plain text. A later phase adds Markdown rendering for headings, emphasis, lists, links, and code-style cue sheets while retaining readable plain-text fallback behavior for malformed input.
...
- Auto-mute policy.
- Playback delay policy.
- Balance curve.
- Playlist resolving.
- Queue building.
- Markdown instruction rendering model.
...
- Count-in/playback delay can be started, cancelled, and completed through app controls and external controls.
...
- Count-in/playback delay countdown and cancellation.
...
- Add auto-mute.
- Add playback delay engine state with the setting disabled by default.
...
### Phase 7: Markdown Instructions

Goal: make instruction files easier to read without changing track metadata.

- Add Markdown rendering for instruction files.
- Support headings, emphasis, lists, links, block quotes, and inline/fenced code.
- Keep malformed Markdown readable instead of failing the screen.
- Add a plain-text fallback path for renderer errors.
- Add tests for Markdown and plain-text instruction files.

### Phase 8: Count-In And Playback Delay Polish

Goal: make the playback delay useful in real practice sessions.

- Add user controls for enabling playback delay and choosing duration.
- Persist playback delay settings.
- Show countdown state in the player screen.
- Ensure play/pause/next/previous from Bluetooth, lock screen, notification, and app all cancel or complete delay consistently.
- Add device smoke tests for delay with background playback and locked screen.
...
- Markdown instructions render clearly with plain-text fallback.
...
- Metronome, waveform, or tempo tools.
